<p align="center">
  <img src="./ChatGPT%20Image%20Aug%2018,%202026,%2010_11_21%20PM.png"
       alt="Varshita AI Engineer Banner"
       width="100%">
</p>

<p align="center">
<a href="https://www.linkedin.com/in/varshita-pallapothu-659256335">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://varshita-portfolio-one.vercel.app/">
<img src="https://img.shields.io/badge/Portfolio-FFD700?style=for-the-badge&logo=vercel&logoColor=black"/>
</a>

<a href="./Varshita_Pallapothu_Resume_AI_FullStack_Intern%20(1).pdf">
<img src="https://img.shields.io/badge/Resume-28A745?style=for-the-badge&logo=adobeacrobatreader&logoColor=white"/>
</a>

<a href="mailto:varshita101206@gmail.com">
<img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://github.com/Varshita1210">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>
</p>


---

<h1 align="center">Hi 👋, I'm Varshita Pallapothu</h1>

<h3 align="center">
AI Engineer • Machine Learning Enthusiast • Full Stack Developer
</h3>

<p align="center">
🎓 B.Tech CSE (Artificial Intelligence & Machine Learning) <br>
🏫 Mahatma Gandhi Institute of Technology (MGIT), Hyderabad <br>
💡 Passionate about building AI-powered applications and scalable web solutions.
</p>

---



### 🚀 About Me

- 🌱 Currently learning **Java, DSA, Machine Learning & Full Stack Development**
- 🤖 Interested in **Artificial Intelligence, NLP, Generative AI and Computer Vision**
- 💻 Building **AI-powered web applications using React, Django, Flask & Firebase**
- 🏆 Active participant in hackathons and AI innovation challenges
- 🎯 Goal: Become an **AI Engineer** at a top product company


## 💻 Tech Stack

<p align="center">

<img src="https://skillicons.dev/icons?i=python,java,c,html,css,javascript,typescript,react,vite,tailwind,flask,django,firebase,tensorflow,sklearn,mysql,sqlite,git,github,vscode" />

</p>
---
---

# 🚀 Featured Projects

<div align="center">

## 🍳 Smart Expiry Scanner AI

### AI-Powered Smart Kitchen Ecosystem

</div>

> An AI-powered food and medicine expiry management ecosystem that helps users scan products, track expiry dates, reduce food waste, generate recipes, and manage their pantry intelligently.

### ✨ Highlights

- 🤖 **AI OCR** for expiry-date detection
- 📦 **Barcode Intelligence** using OpenFoodFacts
- 🧠 **Google Gemini AI** for intelligent analysis
- 🍳 **AI Recipe Generator**
- 🗓️ **Smart Expiry Calendar**
- 🔔 **Smart Notifications**
- 📊 **Waste Prediction & Analytics**
- 🎙️ **Voice Cooking Assistant**
- 💬 **AI Chat Assistant**
- 🥗 **AI Nutrition Analyzer**
- 🛒 **AI Grocery Assistant**
- 👨‍👩‍👧 **Family Sharing**
- 📱 **PWA + Offline Support**

### 🛠️ Tech Stack

`React 19` `TypeScript` `Vite` `Tailwind CSS` `Framer Motion` `Gemini AI` `Firebase` `OpenFoodFacts` `Recharts`

<div align="center">

**🔗 [View Project](https://github.com/HackIndiaXYZ/hackindia-spark-11-hyderabad-telangana-south-central-region-dual-impact)**

</div>

---

## 🎵 AI Music Generation System

<div align="center">

**Deep Learning • LSTM • MIDI Generation**

</div>

An AI system that learns musical patterns from MIDI compositions and generates new musical sequences using an LSTM neural network.

### ✨ Highlights

- 🎼 MIDI dataset processing
- 🧠 LSTM Neural Network
- 🎹 Music sequence prediction
- 🔊 AI-generated MIDI output
- 🧮 Sequence learning
- 🐍 Python + TensorFlow/Keras

### 🛠️ Tech Stack

`Python` `TensorFlow` `Keras` `NumPy` `Music21` `LSTM`

<div align="center">

**🔗 [View Project](https://github.com/Varshita1210/codealpha_ai_music_generation)**

</div>

---

## 🎬 Movie Genre Classification

<div align="center">

**NLP • TF-IDF • Machine Learning**

</div>

An NLP-based machine learning system that predicts a movie genre from its plot summary across **27 genres**.

### 📊 Model Comparison

| Model | Accuracy |
|---|---:|
| Logistic Regression | **58%** |
| Linear SVM | **57%** |
| Multinomial Naive Bayes | **52%** |

### 🛠️ Tech Stack

`Python` `Pandas` `NumPy` `NLTK` `Scikit-learn` `TF-IDF`

<div align="center">

**🔗 [View Project](https://github.com/Varshita1210/Movie_Genre_Classification-Codsoft)**

</div>

---

## 💳 Credit Card Fraud Detection

<div align="center">

**Machine Learning • Classification • Fraud Detection**

</div>

A machine learning system that detects fraudulent credit card transactions and compares multiple classification algorithms.

### 📊 Model Performance

| Model | Accuracy |
|---|---:|
| Logistic Regression | **95.36%** |
| Decision Tree | **99.45%** |
| Random Forest | **99.57%** |

### 🛠️ Tech Stack

`Python` `Pandas` `NumPy` `Scikit-learn` `Matplotlib` `Seaborn`

<div align="center">

**🔗 [View Project](https://github.com/Varshita1210/CreditCardFraudDetection-Codsoft)**

</div>

---

## 🌐 Personal Portfolio

<div align="center">

### My Developer Portfolio

Showcasing my projects, skills, AI/ML work and frontend development.

**[🚀 Visit My Portfolio](https://varshita-portfolio-one.vercel.app/)**

</div>

---

<div align="center">

### 💡 Building ideas into real-world applications.

</div>

---
---

<h2 align="center">🔥 GitHub Streak</h2>

<p align="center">
  <img
    src="https://streak-stats.demolab.com?user=Varshita1210&theme=dark&hide_border=true&border_radius=10"
    alt="Varshita's GitHub Streak"
  />
</p>

